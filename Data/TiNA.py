...
class FMDL_21_PT_TransferNormalsPanel:
    bl_label = 'Normals'
...
class FMDL_TransferNormals(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.transfer_vcol:
            data_types.add('COLOR_CORNER' if 'COLOR_CORNER' in bpy.types.DataTransferModifier.bl_rna.properties['data_types_loops'].enum_items.keys() else 'VCOL')
        ...
        for ob in selection:
            if active != ob and ob.type == "MESH":
                ...
                if self.to_active or len(selection) == 2:
                    transfer.vertex_group = '' if self.group == 'NONE' else self.group
