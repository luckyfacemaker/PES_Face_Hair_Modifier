...
def addTexture(context, blenderMaterial, textureRole, texture, textureIDs, uvMapColor, uvMapNormals, textureSearchPath, loadTextures):
	...
	if identifier in textureIDs:
		blenderTexture = blenderMaterial.node_tree.nodes.get(textureIDs[identifier])
	else:
		...
		filename = findTexture(texture, textureSearchPath) if loadTextures else None
		if filename is None:
		...
		uv_node = blenderMaterial.node_tree.nodes.new('ShaderNodeUVMap')
		uv_node.uv_map = uvMapNormals if '_NRM' in textureRole or 'NormalMap' in textureRole else uvMapColor
		blenderMaterial.node_tree.links.new(uv_node.outputs['UV'], blenderTexture.inputs['Vector'])
		...
		if 'Base_Tex_SRGB' in textureRole or 'Base_Tex_LIN' in textureRole:
			...
			if blenderImage.alpha_mode != 'NONE':
				link_alpha(blenderMaterial, blenderTexture.outputs['Alpha'], principled.inputs['Alpha'])
			if 'pes3DDF_Hair' in blenderMaterial.fmdl_material_technique:
				link_alpha(blenderMaterial, blenderTexture.outputs['Alpha'], principled.inputs['Alpha'])
